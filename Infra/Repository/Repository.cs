...
namespace Infra.Repository
{
    public class Repository<T> : IRepository<T> where T : class
    {
...

        {
            try
            {
                return _dbContex.Set<T>().AsNoTracking().AsQueryable();
            }
            catch(Exception e)
            {
                return null;
        ...
        {
            using (CarWaterLessContext entityContext = new CarWaterLessContext())
            {
        ...
        //public List<T> InsertReturnListGetRecord(List<T> entities)
        //{
        //    using (MovieSellingContext entityContext = new MovieSellingContext())
        //    {
        //        List<T> newEntityList = new List<T>();
        //        for (int i = 0; i < entities.Count(); i++)
        //        {
        //            T newEntity = entityContext.Set<T>().Add(entities[i]);
        //            try
        //            {
        ...
        public List<T> updateMultipleWithGetRecord(List<T> entities)
        {
            using (CarWaterLessContext entityContext = new CarWaterLessContext())
            {
                List<T> newEntityList = new List<T>();
                for (int i = 0; i < entities.Count(); i++)
                {
                    _dbContex.Entry<T>(entities[i]).State = EntityState.Modified;
                    newEntityList.Add(entities[i]);
                }
                var result = entityContext.SaveChanges();
                if (result > 0)
                {
        ...
        public int DeleteList(List<T> entities)
        {
            using (CarWaterLessContext entityContext = new CarWaterLessContext())
            {
                ...
                for (int i = 0; i < entities.Count(); i++)
                {
                    try
                    {
                        T newEntitydelete = entityContext.Set<T>().Attach(entities[i]);
                        T newEntity = entityContext.Set<T>().Remove(entities[i]);
                        newEntityList.Add(newEntity);
                    }
                    catch(Exception e)
                    {
